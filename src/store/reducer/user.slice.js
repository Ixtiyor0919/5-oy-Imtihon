...
export const editUsers = createAsyncThunk("users/editUsers", async function ({id, title, body, categoryName, imageUrl}, {rejectWithValue}) {
    try {
        const { token }  = JSON.parse(localStorage.getItem('token'))
        const res = await fetch(`${Base_URL}/posts/${id}`, {
            method: 'PUT',
            ...
            body: JSON.stringify({
                title, body, categoryName, imageUrl,
            }),
        });
        if(!res.ok) {
            throw new Error('Server errror: ', res.status)
